...
bool BMA253Component::read_int16_le(uint8_t start, int16_t *values, size_t len) {
  ...
  for (size_t i = 0; i < len; i++) {
    values[i] = (int16_t) ((data[i * 2 + 1] << 8) | (data[i * 2]));
  }
  return true;
}
...
void BMA253Component::setup() {
  ...
  if (!this->write_byte(REG_PMU_LPW, (uint8_t) PMU_LPW::NORMAL)) {
    this->mark_failed();
    return;
  }

  if (!this->write_byte(REG_BW_SELECT, (uint8_t) PMU_BW::HZ_7_81)) {
    this->mark_failed();
    return;
  }

  if (!this->write_byte(REG_PMU_RANGE, (uint8_t) PMU_RANGE::G_2)) {
    this->mark_failed();
    return;
  }

  if (!this->write_byte(REG_INT_EN_0, 0b01000000)) {
    this->mark_failed();
    return;
  }
}
